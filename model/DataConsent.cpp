...
bool DataConsent::fromJson(const web::json::value& val)
{
    ...
    if(val.has_field(utility::conversions::to_string_t(U("documents"))))
    {
        ...
        if(!fieldValue.is_null())
        {
            std::vector<std::shared_ptr<DataConsentDocument>> refVal_documents;
...
bool DataConsent::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    ...
    {
        utility::string_t refVal_requestId;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(U("requestId"))), refVal_requestId );
        setRequestId(refVal_requestId);
    }
